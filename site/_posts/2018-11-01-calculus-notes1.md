...
3. 设$X$为数集，$x_0\in X$而$f,g:X\to\mathbb{R}$：
  ...
  2. 局部保序：设$f,g$在点$x_0$处连续
    1. $f(x_0)>g(x_0)\Rightarrow\exists\delta>0\forall x\in B_X(x_0,\delta),f(x)>g(x)$；
    2. $\exists\delta>0\forall x\in B_X(x_0,\delta),f(x)\geq g(x)\Rightarrow f(x_0)\geq g(x_0)$。
  3. 局部保号：上述$g$恒为0的特殊情况；
